...
router.post('/admin/proof/:id/review', async (req, res) => {
  ...
  if (action === 'approve') {
    ...
    if (task) {
      ...
      require('../services/utils').notifyUser(proof.userId, 'Task approved', `Your submission for "${task.title}" w. A Service Reward of $${(task.price/100).toFixed(2)} h credited to your Rewards Wallet.`).catch(console.error);
    }
    res.json({ success: true });
  } else {
    await prisma.taskProof.update({ where: { id }, data: { status: 'rejected', reason, reviewedAt: new Date() } });
    require('../services/utils').notifyUser(proof.userId, 'Task rejected', `Your submission w. Reason: ${reason || 'See reviewer notes.'}`).catch(console.error);
    res.json({ success: true });
  }
});

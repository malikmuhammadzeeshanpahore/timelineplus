// Load header and handle back navigation for Campaigns page
(async () => {
  try {
    console.log('🔄 [CAMPAIGNS] Initializing page...');
    
    // Check authentication
    const token = localStorage.getItem('token');
    if (!token) {
      console.log('❌ [CAMPAIGNS] No token found, redirecting to login');
    ...
    console.log('🔗 [CAMPAIGNS] Fetching header...');
    ...
    console.error('❌ [CAMPAIGNS] Error loading header:', e.message);
  }
})();

// Add back button functionality
document.addEventListener('DOMContentLoaded', () => {
  console.log('📄 [CAMPAIGNS] Page DOM loaded');
  ...
  if (header) {
    ...
    backBtn.addEventListener('click', (e) => {
      e.preventDefault();
      console.log('↩️ [CAMPAIGNS] Back button clicked');

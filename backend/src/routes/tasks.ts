...
router.post('/:id/start', jwtMiddleware, async (req: any, res) => {
...
router.post('/:id/proof', jwtMiddleware, async (req: any, res) => {
  const id = Number(req.params.id);
...
router.post('/admin/proof/:id/review', async (req, res) => {
  ...
  if (action === 'approve') {
    ...
    if (task) {
      ...
      await import('../services/utils').then(m => m.notifyUser(proof.userId, 'Task approved', `Your submission for "${task.title}" was approved. A Service Reward of $${(task.price/100).toFixed(2)} has been credited to your Rewards Wallet.`));
    }
    res.json({ success: true });
  } else {
    await prisma.taskProof.update({ where: { id }, data: { status: 'rejected', reason, reviewedAt: new Date() } });
    await import('../services/utils').then(m => m.notifyUser(proof.userId, 'Task rejected', `Your submission was rejected. Reason: ${reason || 'See reviewer notes.'}`));
    res.json({ success: true });
  }
});

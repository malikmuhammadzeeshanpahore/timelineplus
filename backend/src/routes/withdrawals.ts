...
import { getUserEarningsStatus, getLockConfig } from '../services/trust-score';
  ...
  try {
    ...
    await prisma.notification.create({
      data: {
        ...
        body: `Your withdrawal of $${withdrawal.amount / 100} has been approved and will be processed soon.`
      }
    });
  ...
  try {
    ...
    await prisma.notification.create({
      data: {
        ...
        body: `Your withdrawal of $${withdrawal.amount / 100} has been rejected. Reason: ${reason || 'No reason provided'}`
      }
    });

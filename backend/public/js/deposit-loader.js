// Load header and handle back navigation for Deposit page
(async () => {
  try {
    console.log('🔄 [DEPOSIT] Initializing page...');
    
    // Check authentication
    const token = localStorage.getItem('token');
    if (!token) {
      console.log('❌ [DEPOSIT] No token found, redirecting to login');
    ...
    console.log('🔗 [DEPOSIT] Fetching header...');
    ...
    console.error('❌ [DEPOSIT] Error loading header:', e.message);
  }
})();

// Add back button functionality
document.addEventListener('DOMContentLoaded', () => {
  console.log('📄 [DEPOSIT] Page DOM loaded');
  ...
  if (header) {
    ...
    backBtn.addEventListener('click', (e) => {
      e.preventDefault();
      console.log('↩️ [DEPOSIT] Back button clicked');

  ...
  if (user.trustScore <= config.banThreshold && !user.isBanned) {
    ...
    await prisma.notification.create({
      data: {
        ...
        body: `Your account h banned due to low trust score. Pay ${unlockCost / 100} to unlock.`
      }
    });
 ...
 */
module.exports.getLockConfig(trustScore): {
  lockDays;
  maxWithdraw | null;
  description;
} {
 ...
 */
async function createEarningsLock(
  userId,
  amount,
 ...
 */
async function adminIncreaseTrustScore(
  userId,
  increase,
  adminId,
 ...
 */
async function adminDecreaseTrustScore(
  userId,
  decrease,
  adminId,
  reason = 'Admin adjustment'
 ...
 */
async function unlockBannedAccount(userId, paymentAmount) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new Error('User not found');
  ...
  await prisma.notification.create({
    data: {
      ...
      body: 'Your account h successfully unlocked. Your trust score h reset to 70%.'
    }
  });
 ...
 */
async function getTrustScoreStats(userId) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new Error('User not found');

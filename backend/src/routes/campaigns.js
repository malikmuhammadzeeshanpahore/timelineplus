...
const { autoVerifyWithRetry } = require('../services/verification');
const { performOCRVerification, mockOCRVerification, saveImageTemporarily, deleteTemporaryImage, calculateTimeElapsed } = require('../services/ocr');
const { applyEarlyExitPenalty, createEarningsLock, getUserEarningsStatus, getLockConfig } = require('../services/trust-score');
...
router.get('/my-campaigns', jwtMiddleware, async (req, res) => {
  try {
    ...
    const campaignsWithProgress = campaigns.map(c => ({
      ...
      taskStatus: {
        pending: c.campaignTasks.filter((t) => t.status === 'pending').length,
        assigned: c.campaignTasks.filter((t) => t.status === 'assigned').length,
        verified: c.campaignTasks.filter((t) => t.status === 'verified').length,
        paid: c.campaignTasks.filter((t) => t.status === 'paid').length
      }
    }));
          ...
          if (verificationResult.verified) {
            ...
            await prisma.notification.create({
              data: {
                ...
                body: `Your task w. $${rewardPerTask / 100} added (locked based on trust score).`
              }
            });
          } else {
